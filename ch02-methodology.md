...
- ![図2-15 スケーラビリティテストの結果](./images/ch02/figure-2-15.png)
...
- ユニバーサルスケーラビリティ法則（Universal Salability Law: USL）
  - [ニール・ガンサー博士](https://en.wikipedia.org/wiki/Neil_J._Gunther)がコヒーレンス(👩‍💻[キャッシュコヒーレンシ](https://ja.wikipedia.org/wiki/%E3%82%AD%E3%83%A3%E3%83%83%E3%82%B7%E3%83%A5%E3%82%B3%E3%83%92%E3%83%BC%E3%83%AC%E3%83%B3%E3%82%B7))による遅れのためのパラメータを追加して編み出したもの。
...
- ○で描かれている最初の10 個のデータポイントは、モデルから得られたものである。×で描かれている次の10 個のデータポイントは、モデルの予測値を実測値でチェックしたもの
...
- パフォーマンス指標に基づいてクラウドのスケーリングを自動化するテクノロジ
  - 図2-21 AWSのASG（auto scaling group）
    - ![図2-21 ASG](./images/ch02/figure-2-21.png)
...
- 👩‍💻 個々の値が持つ相対的な影響が考慮され、極端な値が全体の平均に与える影響が抑制される。
...
- 👩‍💻 逆数を使用するため、小さな値が全体の平均に大きな影響を与える。
- 👩‍💻 特定の状況や問題において有用な平均手法
...
- ![図2-23 レイテンシの分布](./images/ch02/figure-2-23.png)
...
### 2.8.6 外れ値
- 外れ値（outlier）
  - 極端に高かったり低かったりして想定される分布にぴったり適合するようには見えない値
- 正規分布では、外れ値があると、平均は少しずれるが中央値はずれない
- 標準偏差と99 パーセンタイルの方が外れ値を見つけやすい
- ヒストグラムを使って分布全体を細かく見るなどし、ふるまいを理解する。

## 2.9 モニタリング
- 時系列的なパフォーマンス統計を記録し、過去と現在を比較
- 時間帯による使用パターンを理解する
- キャパシティプランニング、成長の定量化、使用のピークの確認
### 2.9.1 時系列的なパターン
- 図2-24、図2-25、図2-26 は、時系列的なパターンの例
- ![図2-24,25,26 アクティビティのモニタリング](./images/ch02/figure-2-24.png)
  - 1 時間ごと
  - 毎日: 勤務時間（午前9 時から午後5 時）に一致する日次的なパターンが見られることがある。
  - 毎週: 日次パターンと同様に、勤務日と週末の休みによって毎週同じパターンが現れる場合がある。
  - 四半期ごと: 財務諸表は、四半期ごとに作られる。
...
## 2.10 ビジュアライゼーション
- ビジュアライゼーション（visualization、可視化）
  - テキストよりも多くのデータを解析できる
  - パターン認識やパターンマッチングもできる
  - 可視化すれば相関関係が簡単にわかる場合がある

### 2.10.1 折れ線グラフ
- 折れ線グラフ（line chart、line graph）
  - 経過時間をx軸として、パフォーマンス指標を時系列的に検証するために使われる
- 図2-28 は1 秒ごとの中央値、1 標準偏差、パーセンタイル
  - このグラフは、平均が普通に予想されるよりも高くなっている理由を示している
- ![図2-28 中央値、平均、標準偏差、パーセンタイル](./images/ch02/figure-2-28.png)

### 2.10.2 散布図
- 図2-29 は、同じ範囲のディスクI/O イベントを散布図（scatter plot）の形で示したもの
- ![図2-29 散布図](./images/ch02/figure-2-29.png)
- 散布図はすべてのデータを示している。
- 平均レイテンシの値が予想よりも高めになっている理由が完全にわかる。
- 外れ値の存在も明らかになる。
- 点が重なり合ってしまって区別しにくい。

### 2.10.3 ヒートマップ
- ヒートマップ（heat map、「列量子化グラフ: column quantization」という名前の方が適切だろう）
  - x、y 軸の一定の範囲をバケット（bucket）と呼ばれるグループに量子化することによって散布図のスケーラビリティの問題を解決している。
- コンピューティングのレイテンシ、使用率などの指標のためにヒートマップを使うのは私が考え出したことだ。
  - 2008 年にリリースされたSun Microsystems のZFS ストレージ・アプライアンスの分析で初めて使われた。
  - 今ではGrafanaなどのパフォーマンスモニタリングプロダクトで普通に使われている。
- ![図2-30 ヒートマップ](./images/ch02/figure-2-30.png)
  - 大部分のデータによるパターンが明らかになってくる
- 図2-30 ディスクI/O トレーシングの完了時の範囲全体
- ![図2-31 ヒートマップ：全範囲](./images/ch02/figure-2-31.png)
  - 時間の範囲は9 倍になっているが、グラフはまだとても読みやすい。
    - 範囲の大部分で二峰分布が見られる。
### 2.10.4 タイムライン図
- タイムライン上の棒という形でさまざまなアクティビティを示す。
- 主としてフロントエンドパフォーマンス分析（ウェブブラウザ）でネットワーク要求の時間関係を示すために広く使われる。
- 滝グラフ（waterfall chart）とも呼ばれる。
- ![図2-32 Firefoxのタイムライン図](./images/ch02/figure-2-32.png)
- バックエンドパフォーマンス分析（サーバー）にも使われる。
  - KernelShark, Trace Compass

### 2.10.5 表面プロット
- 3次元データを3次元サーフェスにレンダリングして表現
  - 👩‍💻 [表面プロット](https://jp.mathworks.com/help/matlab/ref/surf.html)
- 第3次元の値がひんぱんに大きく変動しないときにもっとも効果的で、緩やかな起伏のある丘陵地のようなイメージになる。
- 表面プロットは、ワイヤフレームモデル（wireframe model）としてレンダリングされることが多い。
  - 👩‍💻 ワイヤフレームモデル: 3DCADや3DCGで立体図形を輪郭線のみで表現する手法
- 図2-33 ワイヤフレームタイプの表面プロット:データセンタのCPU使用率
- ![図2-33 ワイヤフレームタイプの表面プロット:データセンタのCPU使用率](./images/ch02/figure-2-33.png)
  - 多くのサーバーから得られた秒ごとの値が60 秒分含まれている（300 台の物理サーバー、5,312 個のCPUから構成されるデー
タセンター全体のイメージから切り出した）。
- 色調、彩度を使って第4、第5次元のデータをビジュアライゼーションに追加できる。
  - 解像度が十分であれば、第6次元としてパターンを使うこともできる。

### 2.10.6　ビジュアライゼーションツール
- Unixのパフォーマンス分析は、歴史的にテキストベースのツールが多用されてきた。
  - 高速、リアルタイムに実行できる。
- 新しいビジュアライゼーションツールはリアルタイムでシステムパフォーマンスを表示でき、ブラウザやモバイルデバイスからアクセスできる。
